...
export default function AdditionalInfo() {
    ...
    const fieldKeys = [
        ...
        'streetAddress', 'city', 'state', 'zipCode', 'customerPropertySize',
        'selectedPrefTime', 'selectedEquipmentItems', 'selectedPropertySizes',
        'crewSize', 'radiusSize', 'documentsUploaded'
    ];
    ...
    //Customer details
    const [customerPropertySize, setCustomerPropertySize] = useState<'Tiny' | 'Small' | 'Medium'| 'Large' | 'Extra Large' | ''>('');
    const [hasSteps, setHasSteps] = useState(true);
    const [isPetFriendly, setIsPetFriendly] = useState(true);
    const [selectedCleaningSpecifics, setSelectedCleaningSpecifics] = useState<string[]>([]);
    ...
    const handleRadiusSelect = (size: 'Tiny' | 'Small' | 'Medium'|
    'Large' | 'Extra Large') => {
        setRadiusSize(size);
        closeRadiusMenu();
    }
    const handleCrewSelect = (size: 'Tiny' | 'Small' | 'Medium'|
    'Large' | 'Extra Large') => {
        setCrewSize(size);
        closeCrewMenu();
    }
    const handlePropertySelect = (size: 'Tiny' | 'Small' | 'Medium'|
    'Large' | 'Extra Large') => {
    ...
    const cleaningSpecificsOptions = [
    ...
    const DocumentUploadSection = ({ sectionKey, title }: { sectionKey: DocumentSectionKey; title: string; }) => {
        ...
        return (
            <View style={styles.documentSection}>
                ...
                {docs.length > 0 && (
                    ...
                    <Text variant="bodyMedium">Documents will be uploaded on submission.</Text>
                    {docs.map((doc, index) => (
    ...
    const validateField = (key: string) => {
        switch (key) {
            ...
            case 'email': return !email.trim() || !email.includes('@');
            case 'phoneNumber': return !phoneNumber.trim() || phoneNumber.length < 10;
            case 'userRole': return !userRole.trim();
            case 'streetAddress': return !streetAddress.trim();
            case 'city': return !city.trim();
            case 'state': return !state.trim();
            case 'zipCode': return !zipCode.trim() || zipCode.length < 5;
            case 'customerPropertySize': return userRole === 'Customer' && !customerPropertySize;
            case 'selectedPrefTime': return userRole === 'Customer' && selectedPrefTime.length === 0;
            case 'selectedEquipmentItems': return userRole === 'Contractor' && selectedEquipmentItems.length === 0;
            case 'selectedPropertySizes': return userRole === 'Contractor' && selectedPropertySizes.length === 0;
            case 'crewSize': return userRole === 'Contractor' && !crewSize;
            case 'radiusSize': return userRole === 'Contractor' && !radiusSize;
            case 'documentsUploaded': return userRole === 'Contractor' && Object.entries(documentsBySection).some(([_, docs]) => docs.length === 0);
    ...
    const handleContractorPref = async() => {
        ...
        const userProfile = {
            uid, firstName, lastName, email, phoneNumber, role: userRole, 
            streetAddress, city, state, zipCode, crewSize, prefPropertySizeWork: selectedPropertySizes, 
            prefRadiusWork: radiusSize, equipments: selectedEquipmentItems,
            customerPropertySize, hasPropertySteps: hasSteps, usePetFriendlyMaterial: isPetFriendly, 
            cleaningSpecifics: selectedCleaningSpecifics, prefTime: selectedPrefTime
        }
        ...
        for (const [index, doc] of allDocuments.entries()) {
            try {
                ...
                console.error(`Failed to process file ${doc.name ?? `file-${index}`}:`, error);
            }
        }
        ...
        api.post(`/users/create`, formData, {
            ...
            alert(`Failed to upload: ${error.status}`);
        });
    };
    ...
    const renderRoleSelector = () => {
        ...
        return (
            <View style={styles.inputContainer}>
                <Text variant="headlineSmall" style = {styles.rolesText}>Select Your Role</Text>
    ...
    return (
        <ScrollView contentContainerStyle={styles.scrollContainer}>
            <View style={styles.info}>
                <Text variant="headlineMedium">Additional Information</Text>
                <View style={styles.infoTextBox}>
                    ...
                    {renderField('phoneNumber', 'Phone Number', phoneNumber, setPhoneNumber)}
                    <Text variant="headlineSmall">Address</Text>
                    {renderField('streetAddress', 'Street Address', streetAddress, setStreetAddress)}
                    {renderField('city', 'City', city, setCity)}
                    {renderField('state', 'State', state, setState, styles.stateInput)}
                    {renderField('zipCode', 'Zip Code', zipCode, setZipCode, styles.zipInput)}
                    {renderRoleSelector()}
                </View>

                {/* Documentation section */}
                {userRole === 'Contractor' && <Text variant="headlineMedium">Upload required documents</Text>}
                {userRole === 'Contractor' &&
                    (Object.entries(DOCUMENT_SECTIONS) as [DocumentSectionKey, string][]).map(
                        ([key, label]) => (
                ...
                {userRole === 'Customer' && (
                    <>
                        <Text variant="headlineMedium" style = {styles.promptText}>Select Your Property Size</Text>
                        <View style={{ paddingTop: 5, flexDirection: 'row', justifyContent: 'center' }}>
                            <Menu
                                visible={propertyVisible}
                                onDismiss={closePropertyMenu}
                                anchor={
                                    <Button mode="outlined" onPress={openPropertyMenu} style={[showErrors && validateField('customerPropertySize') && { borderColor: 'red' }]}>
                                        {customerPropertySize || 'Select Property Size'}
                                    </Button>
                                }>
                                <Menu.Item onPress={() => handlePropertySelect('Tiny')} title="Tiny (0-500 sq ft)" />
                                <Menu.Item onPress={() => handlePropertySelect('Small')} title="Small (501-1,501 sq ft)" />
                                <Menu.Item onPress={() => handlePropertySelect('Medium')} title="Medium (1,501-3,000 sq ft)" />
                                <Menu.Item onPress={() => handlePropertySelect('Large')} title="Large (3,001-6,000 sq ft)" />
                                <Menu.Item onPress={() => handlePropertySelect('Extra Large')} title="Extra Large (6,001+ sq ft)" />
                            </Menu>
                        </View>
                        ...
                        <Text variant="headlineMedium" style = {styles.promptText}>Select Cleaning Specifics</Text>
                        {cleaningSpecificsOptions.map(item => (
                        <Checkbox.Item
                            key={item}
                            label={item}
                            status={selectedCleaningSpecifics.includes(item) ? 'checked' : 'unchecked'}
                            onPress={() => toggleCleaningSpecifics(item)}
                        />
                        ))}

                        <Text variant="headlineMedium" style = {styles.promptText}>Select Your Preferred Time</Text>
                        {showErrors && validateField('selectedPrefTime') && (
                            <HelperText type="error">Select at least one preferred time</HelperText>
                        )}
                        {prefTimeOptions.map((item) => (
                        <Checkbox.Item
                            key={item.key}
                            label={item.label}
                            status={selectedPrefTime.includes(item.key) ? 'checked' : 'unchecked'}
                        ...
                        <Text variant="headlineMedium" style = {styles.promptText}>Does your property have steps?</Text>
                        <View style = {styles.prompts}>
                            <Button
                                mode="outlined"
                                onPress={() => setHasSteps(true)}
                                style = {[styles.infoButton, hasSteps === true && styles.selectedButton]}
                                labelStyle = {[styles.buttonLabel, hasSteps === true && styles.selectedButtonLabel]}
                            > Yes </Button>

                            <Button
                                mode="outlined"
                                onPress={() => setHasSteps(false)}
                                style={[styles.infoButton, hasSteps === false && styles.selectedButton]}
                                labelStyle = {[styles.buttonLabel, hasSteps === false && styles.selectedButtonLabel]}
                            > No </Button>
                        </View>

                        <Text variant="headlineMedium" style = {styles.promptText}>Would you like pet friendly materials used?</Text>
                        <View style = {styles.prompts}>
                            <Button
                                mode="outlined"
                                onPress={() => setIsPetFriendly(true)}
                                style = {[styles.infoButton, isPetFriendly === true && styles.selectedButton]}
                                labelStyle = {[styles.buttonLabel, isPetFriendly === true && styles.selectedButtonLabel]}
                            > Yes </Button>

                            <Button
                                mode="outlined"
                                onPress={() => setIsPetFriendly(false)}
                                style={[styles.infoButton, isPetFriendly === false && styles.selectedButton]}
                                labelStyle = {[styles.buttonLabel, isPetFriendly === false && styles.selectedButtonLabel]}
                            > No </Button>
                        </View>
                ...
                <>
                    ...
                    <View style={{ paddingTop: 5, flexDirection: 'row', justifyContent: 'center' }}>
                        <Menu
                            ...
                            anchor={
                            <Button mode="outlined" onPress={openCrewMenu} style={[showErrors && validateField('crewSize') && { borderColor: 'red' }]}>
                                {crewSize || 'Select Crew Size'}
                            </Button>
                            }>
                            <Menu.Item onPress={() => handleCrewSelect('Tiny')} title="Tiny (1-2 member(s))" />
                            <Menu.Item onPress={() => handleCrewSelect('Small')} title="Small (3-5 members)" />
                            <Menu.Item onPress={() => handleCrewSelect('Medium')} title="Medium (6-9 members)" />
                            <Menu.Item onPress={() => handleCrewSelect('Large')} title="Large (10-13 members)" />
                            <Menu.Item onPress={() => handleCrewSelect('Extra Large')} title="Extra Large (14+ members)" />
                        </Menu>
                    </View>

                    <Text variant="headlineSmall">Size of Properties to Work on</Text>
                    {showErrors && validateField('selectedPropertySizes') && (
                        <HelperText type="error">Select at least one preferred property size to work on</HelperText>
                    )}
                    {propertySizeOptions.map(item => (
                    <Checkbox.Item
                        key={item}
                        label={item}
                        status={selectedPropertySizes.includes(item) ? 'checked' : 'unchecked'}
                    ...
                    <Text variant="headlineSmall">Preferred Radius to work</Text>
                        ...
                        <Menu
                            ...
                            anchor={
                            <Button mode="outlined" onPress={openRadiusMenu} style={[showErrors && validateField('radiusSize') && { borderColor: 'red' }]}>
                                {radiusSize || 'Select Radius'}
                            </Button>
                            }>
                            <Menu.Item onPress={() => handleRadiusSelect('Tiny')} title="Tiny (1 mi.)" />
                            <Menu.Item onPress={() => handleRadiusSelect('Small')} title="Small (5 mi.)" />
                            <Menu.Item onPress={() => handleRadiusSelect('Medium')} title="Medium (10 mi.)" />
                            <Menu.Item onPress={() => handleRadiusSelect('Large')} title="Large (25 mi.)" />
                            <Menu.Item onPress={() => handleRadiusSelect('Extra Large')} title="Extra Large (50 mi.)" />
                        </Menu>
                    </View>

                    <Text variant="headlineSmall">Type of Equipment</Text>
                    {showErrors && validateField('selectedEquipmentItems') && (
                        <HelperText type="error">Select at least one equipment</HelperText>
                    )}
                    {snowEquipmentOptions.map(item => (
                    <Checkbox.Item
                        key={item}
                        label={item}
                        status={selectedEquipmentItems.includes(item) ? 'checked' : 'unchecked'}
